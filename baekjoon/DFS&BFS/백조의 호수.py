...
# === 알고리즘 설명 ===
"""
핵심 아이디어: 두 개의 BFS를 병렬로 실행

1. 백조 BFS:
   - 현재 백조가 도달할 수 있는 모든 물 공간을 탐색
   - 얼음을 만나면 "다음 날 큐"에 저장 (내일 녹을 예정)
   - 목적지에 도달하면 즉시 종료

2. 물 확산 BFS:
   - 현재 물 위치에서 인접한 모든 얼음을 녹임
   - 녹은 위치들이 다음 날의 물 확산 시작점이 됨

3. 효율성:
   - 매일 필요한 부분만 탐색 (전체 격자 재탐색 X)
   - 시간 복잡도: O(R×C) - 각 칸을 최대 한 번씩만 방문
   - 공간 복잡도: O(R×C) - visited 배열과 큐들
"""