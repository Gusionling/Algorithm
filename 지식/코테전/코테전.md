...
# [1~M]을 N번 뽑는 중복순열
cases = list(product(range(1, M+1), repeat=N))
print(cases)
# [(1,1,1), (1,1,2), (1,2,1), (1,2,2), (2,1,1), (2,1,2), (2,2,1), (2,2,2)]

# 리스트로 변환하려면
cases = [list(case) for case in product(range(1, M+1), repeat=N)]
print(cases)
# [[1,1,1], [1,1,2], [1,2,1], [1,2,2], [2,1,1], [2,1,2], [2,2,1], [2,2,2]]
```

``` python
def generate_cases(N, M):
    result = []
    
    def backtrack(path):
        if len(path) == N:  # N개를 다 뽑으면
            result.append(path[:])
            return
...
딕셔너리나 리스트 모두 행을 삭제하고 싶을 때 del을 쓰거나 pop(값 반환) 을 쓴다 
